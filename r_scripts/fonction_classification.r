extraction_concept <- function(sepSystem,resultatConceptsDir,sepFichier,decFichier,methodeTabName,methodeTabNbreTest,typeMethodeTabName,typeMethodeTabFichier,typeMethodeTabNbreMotMax
	    ,typeMethodeTabNbreMotMin,typeMethodeTabMarge,fichierMots,nbreConceptsExpert,nbreClasseMclustAuto)
{
  library(package=stats)# pour kmeans et hclust
  ...
  for(mehodeNum in 1:methodeTabLength)
  {
    ...
    for(typeMethodeNum in 1:typeMethodeTabLength)
    {
      ...
      #imm <- read.delim(file=fichierImm)      
	...
	for(i in 1:nbreTest)
	{
	  switch(methode,
	    kmeans = 
	    {
	      methodeResultat <- kmeans(x=immUtile,centers=nbreConceptsExpert,iter.max=500)#,nstart=nbreConceptsExpert , Hartigan
	    ...
	    hclust =
	    {
	      methodeResultat <- hclust(dist(immUtile),"ward")#ave
