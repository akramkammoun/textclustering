...
function creationMatImm($corpusPurifieFichier,$motsCorpusAlloueFichier,$nbrePhrasesCorpusPurifie,$immFichier)
{
    ...
    for($j=0;$j<$nbreMotsCorpusAlloue;$j++)
    {
		...
		{
		    $p_i_j = $val_nobar_nobar_i_j /( $nbreOccMot_nobar_i + $nbreOccMot_nobar_j - $val_nobar_nobar_i_j);
		    $p_i = $nbreOccMot_nobar_i / $nbrePhrasesCorpusPurifie;
	   	    $p_j = $nbreOccMot_nobar_j / $nbrePhrasesCorpusPurifie;
		    $immPartie = $p_i_j * log($p_i_j/($p_i * $p_j),2);
		}
		else
	            $immPartie = 0;

		if($immPartie == 0)
			$immPartie = 0.000000001;

            

	    //if(strtoupper($immPartie) == "NAN")
              //    $immPartie .= 0;
